#!/usr/bin/env python3
"""
eval_autotrack_bidir.py — Dense-grid bidirectional autonomous cell tracking.
...
OUT_DIR = Path(os.environ.get(
    "RESULTS_DIR",
    "/home/cps/Documents/biomed/ViT_part_nano/pipeline/benchmark_results/autotrack_bidir"))
...
class CellTrack:
    __slots__ = ("track_id", "memory_bank", "prev_logits", "last_mask",
...
def spawn_track(model, fpn, curr_pos, det, track_id, n_mem, fi, device):
    ...
    mem_bank.append({"feat": mem_out["vision_features"],
                     "pos":  mem_out["vision_pos_enc"][0]})

    return CellTrack(track_id=track_id, memory_bank=mem_bank,
...
def reprompt_track(model, track, fpn, det, n_mem, fi, device):
    ...
    # Reset memory bank with fresh prompt — discard stale degraded memories
    track.memory_bank = _make_memory_bank(max(1, n_mem))
    pred_up = F.interpolate(mask_lr, size=(IMG_SIZE, IMG_SIZE),
                             mode="bilinear", align_corners=False)
    mem_out = model.memory_encoder(fpn[-1], pred_up, skip_mask_sigmoid=False)
    track.memory_bank.append({"feat": mem_out["vision_features"],
                               "pos":  mem_out["vision_pos_enc"][0]})
